...
def setup_rserver(name='rstudio', title='RStudio', config_file=None):
    """
    name:        The name used in entry_points['jupyter_serverproxy_servers'] 
                 of setup.py
    title:       Title of the proxy to be shown on Jupyter's launcher. 
    ...
    def _get_cmd(port,name='rstudio',config_file=None):
        ...
        if _support_arg('www-root-path'):
            cmd.append('--www-root-path={base_url}' + name + '/')
